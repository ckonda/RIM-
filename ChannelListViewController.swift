...
class ChannelListViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (indexPath as NSIndexPath).section == Section.createNewChannelSection.rawValue {
            ...
            cell.textLabel?.text = channels[(indexPath as NSIndexPath).row].channelName//will call upon channel names available in model
        }
        return cell
    }
    ...
    private func observeChannels() {
        ...
        channelRefHandle = channelRef.observe(DataEventType.value, with: { (snapshot) in
            
            self.channels.removeAll()
            for channel in snapshot.children.allObjects as! [DataSnapshot]{
                let id = snapshot.key
                
                let channelNames = channel.value as? [String: AnyObject]
                let name = channelNames?["channelName"] as! String?
                
                self.channels.append(Channel(channelID: id, channelName: name, latestMessageTimeStamp: nil))
            }
